  ...
  function renderTours(page, direccion = false) {
    const renderContent = () => {
      ...
      pageTours.forEach(tour => {
        ...
        tourCard.innerHTML = `<img src="${tour.img}" alt="${tour.titulo}" /><div class="tour-content"><h3>${tour.titulo}</h3><div class="tour-subtitle">${tour.destino}</div><div class="tour-price">Desde: <strong>${tour.precio}</strong></div><div class="tour-details">${tour.detalles}</div><button class="btn-reserve" type="button">Reserve ahora</button></div>`;
